...
class PaperProcessor:
    ...
    def __init__(self,
        ...
        self.output_dir = output_dir or os.path.join(settings.ANALYSIS_REPORT_PATH,
    ...
    def analyze_paper(self,
        ...
        analysis_prompt = (
            "请详细讲一下这篇论文，尤其要关注在研究背景与动机，创新点及方法实现。回答时直接从内容开始，不要包含任何引言句（如'好的，我将详细分析'或'下面是我的分析'等）。"
            "请用中文回答，数学公式请用$或$$包裹。在回答时，请在需要展示的图表位置插入论文中的图表，格式为单独的一行：>>> Figure-X <<<或>>> Table-Y <<<\n"
            f"以下是论文内容：\n{text_data}"
        )
